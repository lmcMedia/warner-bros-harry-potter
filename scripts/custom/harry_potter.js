(function($) {'use strict';
	...
	var has3d, vendor = '', version = '4.0.9', PI = Math.PI, A90 = PI / 2, isTouch = 'ontouchstart' in window, mouseEvents = (isTouch) ? {
		down : 'touchstart',
		move : 'touchmove',
		up : 'touchend',
		over : 'touchstart',
	...
	pagesInDOM = 6, turnMethods = {
		...
		init : function(options) {
			...
			$(this).bind(mouseEvents.down, data.eventHandlers.mouseStart).bind('end', turnMethods._end).bind('pressed', turnMethods._pressed).bind('released', turnMethods._released).bind('flip', turnMethods._flip);

			$(this).parent().bind('start', data.eventHandlers.start);

			$(document).bind(mouseEvents.move, data.eventHandlers.mouseMove).bind(mouseEvents.up, data.eventHandlers.mouseEnd);
		...
		destroy : function() {
			...
			var page, flipbook = this, data = this.data(), events = ['end', 'first', 'flip', 'last', 'pressed', 'released', 'start', 'turning', 'turned', 'zooming', 'missing'];
			...
			$(document).unbind(mouseEvents.move, data.eventHandlers.mouseMove).unbind(mouseEvents.up, data.eventHandlers.mouseEnd);
		...
		// TODO: REMOVED THIS TO FIX BROKEN LINKS ON PAGES, BUT IT DOESNT SOLVE THE MISSING JS CAPTURES ON CLICK
		...
		_pressed : function(e) {
			...
			var page, data = $(e.target).data().f, turn = data.opts.turn, turnData = turn.data(), pages = turnData.pages;
		...
		// This event is called in context of flip
		// TODO: MODIFIED THE outArea = point.x < 0 || point.x > page.width(); TO USE 500 AS THE PAGE TURN POINT (1/4 way from the right)
		...
		_released : function(e, point) {
			...
			if (turnData.display == 'single') {
				...
				outArea = point.x < 500 || point.x > page.width();
			}
		...
		update : function() {
			...
			if (this.turn('animating') && data.pageMv[0] !== 0) {
				...
				var p, apage, fixed, pos = this.turn('calculateZ', data.pageMv), corner = this.turn('corner'), actualView = this.turn('view'), newView = this.turn('view', data.tpage);
				...
				for (page in data.pageWrap) {
					...
					data.pageWrap[page].css({
						...
						zIndex : (data.pageObjs[page].hasClass('hard') ? pos.partZ[page] : pos.pageZ[page]
						) || ( fixed ? -1 : 0)
					});
	...
	flipMethods = {
		...
		_backGradient : function() {
			...
			if (gradient && !data.bshadow)
				data.bshadow = $('<div/>', divAtt(0, 0, 0)).css({
					'position' : '',
					width : this.width(),
					height : this.height()
			...
			if (!data.wrapper)
				switch (data.effect) {
					...
					case 'sheet':
						...
						var width = this.width(), height = this.height(), size = Math.round(Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)));
		...
		_fold : function(point) {
			...
			switch (data.effect) {
				...
				case 'sheet':
					...
					var that = this, a = 0, alpha = 0, beta, px, gradientEndPointA, gradientEndPointB, gradientStartVal, gradientSize, gradientOpacity, shadowVal, mv = point2D(0, 0), df = point2D(0, 0), tr = point2D(0, 0), folding = flipMethods._foldingPage.call(this), tan = Math.tan(alpha), ac = turnData.opts.acceleration, h = data.wrapper.height(), top = point.corner.substr(0, 1) == 't', left = point.corner.substr(1, 1) == 'l', compute = function() {

						var rel = point2D((o.x) ? o.x - point.x : point.x, (o.y) ? o.y - point.y : point.y), middle = point2D((left) ? width - rel.x / 2 : point.x + rel.x / 2, rel.y / 2), alpha = A90 - Math.atan2(rel.y, rel.x), gamma = alpha - Math.atan2(middle.y, middle.x), distance = Math.max(0, Math.sin(gamma) * Math.sqrt(Math.pow(middle.x, 2) + Math.pow(middle.y, 2)));
						...
						var f = ['0', 'auto'], mvW = (width - h) * x[0] / 100, mvH = (height - h) * x[1] / 100, cssA = {
							...
							right : f[c[2]],
							bottom : f[c[3]]
						}, cssB = {}, aliasingFk = (a != 90 && a != -90) ? ( left ? -1 : 1) : 0, origin = x[0] + '% ' + x[1] + '%';
		...
		_eventStart : function(e) {
			...
			if (!data.disabled && !this.flip('isTurning') && data.opts.page == turn.data().pagePlace[data.opts.page]) {
